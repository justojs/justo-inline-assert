...
export function have(obj, props) {
  ...
  if (typeof(props) == "string") {
    ...
    for (let i = 0; res && i < props.length; ++i) {
    ...
    res = keys.length > 0;
    for (let i = 0, keys = Object.keys(props); res && i < keys.length; ++i) {
      let name = keys[i];
      let value = props[name];

      res = obj.hasOwnProperty(name) && equal(obj[name], value);
    }
  }
...
export function haveAny(obj, props) {
  ...
  if (props instanceof Array) {
    ...
    for (let name in props) {
      if (obj.hasOwnProperty(name) && equal(obj[name], props[name])) {
        res = true;
        break;
      }
    }
